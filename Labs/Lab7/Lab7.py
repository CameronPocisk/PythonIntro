...
#RQ3
    def talk_to(self, person):
        ...
        if person in self.place.characters:
            curChar = self.place.characters[person] #Get the person using a dict
            print(curChar.name + " says: " + curChar.talk())
        else:
...
#RQ4
    def take(self, thing):
        ...
        Hotdog is not here.
        """
        #In the last test I had to fix a test becuase it was holding extra spaces
        if type(thing) != str:
            print('Thing should be a string.')
            return None   # Replace this line
        if thing in self.place.things:
            curThing = self.place.things[thing] #Get the person using a dict
            # print(curThing.name + ": " + curThing.description)
            print(self.name + " takes the " + curThing.name)
            self.backpack.append(curThing)
